...
`server/.env` містить бойовий `DATABASE_URL` (Railway), але `server/config/db.js`
і `config/sequelizeConfig.js` читають його, лише коли явно виставлено
`ALLOW_PROD_DB=1`. Без цього прапорця `npm run dev`, `npm run migrate` і всі
`server/scripts/*` фізично не можуть зачепити продакшен — за замовчуванням вони
йдуть у локальний Postgres (`pp_shop_dev`, дефолти `postgres`/`postgres`,
перевизначаються через гітігнорений `server/.env.local`). Перед будь-якою
міграцією: `npm run db:backup` (або `npm run db:export-json`, якщо немає
`pg_dump`) + `npm run db:snapshot`, після — `npm run db:verify` і повторний
знімок. Кожна міграція має перевірений `down`. Якщо міграція чи скрипт справді
мають піти на продакшен — `ALLOW_PROD_DB=1` додається окремо перед конкретною
командою (не глобально в `.env`) і тільки після явного підтвердження
користувача. Повний протокол — `docs/admin-redesign/03-screens.md`, розділ
«Робота з базою».

## Поточна робота

Триває повний редизайн адмінки за `docs/admin-redesign/03-screens.md`, крок за
кроком, екран за екраном (протокол і порядок — там же). **Перед будь-якою
правкою в `src/pages/admin` або `src/features/orders-rental` прочитати
`docs/admin-redesign/00-plan.md`** — нова структура розділів, таблиця
відповідності старих і нових маршрутів. `docs/admin-redesign/01-inventory.md` —
повна інвентаризація чинного функціоналу (що саме має вціліти).
`docs/admin-redesign/prototype.html` — клікабельний прототип цільового
інтерфейсу.

Стан: кроки 1–3, 4.1–4.2 (фундамент, маршрути, каркас, токени, шрифти) і крок
4.3 для Складу й Угоди — виконані й закомічені. Далі за порядком у
`03-screens.md`: Угоди + Календар (без окремої контрольної точки), потім
Каталог (контрольна точка).
